...
import { domain, domainPic } from '../common/commonVar';
...
export function GetJson(url, data) {
  const _data = data ? _.cloneDeep(data) : {};
  return handleRequest(get(`${domain}${url}`, _data));
}

export function PostJson(url, data) {
  return handleRequest(post(`${domain}${url}`, data));
}
